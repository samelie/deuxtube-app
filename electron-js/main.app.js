...
const IS_PROD = process.env.NODE_ENV !== "develop"
...
module.exports = (mainWindow ,tmpPath, IS_PROD) => {
  const HOST = "https://rad.wtf/redis/"

  const getVideoDuration = videoPath => {
    const child = spawn(`${path.join(__dirname, "../bin/ffprobe")}`, [
      `-print_format`,
      `json`,
      `-show_format`,
      `-show_streams`,
  ...
  const uploadComplete = (videoId, localPath, note) => {
    xhr(`${HOST}sadd`, {
      method: "POST",
      json: true,
      body: {
  ...
  ipcMain.on("youtube-upload", (event, arg) => {
    _currentEvent = event
    const { totalByteSize, local, note } = arg
    _currentUploadArgs = Object.assign({}, arg)

    fs.appendFile(
      path.join(tmpPath, "log.txt"),
      `${_currentUploadArgs.toString()}\n`
    )

    _totalByteSize = totalByteSize

    let _cred = JSON.parse(
      fs.readFileSync(path.join(__dirname, "ytcreds.json"))
    )
    console.log(arg.credentials)

    Uploader.init(_cred, arg.credentials).then(() => {
      let options = {
      ...
      return Uploader.upload(
        ...
        .then(uploaded => {
          ...
          uploadComplete(uploaded, arg.local, note)
