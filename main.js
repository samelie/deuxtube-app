...
const redis = exec(`${path.join(__dirname, "bin/redis-server")}`)
redis.stdout.on("data", function(chunk) {
  fs.appendFile(path.join(tmpPath, "log.txt"), `Redis: ${chunk}`)
})

let server = exec(
  `${path.join(__dirname, "bin/node")} ${path.join(
    __dirname,
    chewbPath
  )}`,
  { maxBuffer: NaN },
...
function createWindow() {
  ...
  const AppJs = require("./electron-js/main.app")(mainWindow, tmpPath, IS_PROD)
